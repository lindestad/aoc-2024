...
pub fn recursive_trail_search(
    grid: &[Vec<u8>],
    position: (usize, usize),
    unique_trail_ends: &mut HashSet<(usize, usize)>,
) -> usize {
        ...
        if check_inbounds(
            ...
            if grid[new_position.1][new_position.0] == grid[position.1][position.0] + 1 {
                if grid[new_position.1][new_position.0] == b'9' {
                    if unique_trail_ends.contains(&new_position) {
                        continue;
                    }
                    trail_count += 1;
                    unique_trail_ends.insert(new_position);
                } else {
                    trail_count += recursive_trail_search(grid, new_position, unique_trail_ends);
                }
            }
        }
    }
...
pub fn sum_trail_scores(grid: &[Vec<u8>]) -> usize {
    let trailheads = find_trailheads(grid);
    let mut total_score = 0;

    for trailhead in trailheads {
        total_score += recursive_trail_search(grid, trailhead, &mut HashSet::new());
    }
    total_score
}
