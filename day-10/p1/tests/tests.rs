...
fn test_recursive_trail_search() {
    ...
    assert_eq!(
        recursive_trail_search(&input, (3, 0), &mut HashSet::new()),
        2
    );
    assert_eq!(
        recursive_trail_search(&input, (4, 3), &mut HashSet::new()),
        1
    );
    assert_eq!(
        recursive_trail_search(&input, (1, 1), &mut HashSet::new()),
        0
    );
    assert_eq!(
        recursive_trail_search(&input, (0, 0), &mut HashSet::new()),
        0
    );
    assert_eq!(
        recursive_trail_search(&input, (6, 5), &mut HashSet::new()),
        1
    );
}
...
fn test_sum_trail_scores() {
    ...
    assert_eq!(sum_trail_scores(&input), 2);
    let input2 = vec![
        vec![b'.', b'.', b'.', b'0', b'.', b'0', b'.'],
        vec![b'.', b'.', b'.', b'1', b'.', b'.', b'.'],
        vec![b'.', b'.', b'.', b'2', b'.', b'.', b'.'],
    ...
    assert_eq!(sum_trail_scores(&input2), 2);
    let input3 = vec![
        vec![b'.', b'.', b'.', b'0', b'.', b'0', b'.'],
    ...
    assert_eq!(sum_trail_scores(&input3), 4);
}
